...
def download_google_font(
    ...
    if is_valid(file_path):
        print(f"   ✅ Font '{output_filename}' already present and valid.")
        return True
    ...
    for attempt in range(1, max_retry + 1):
        try:
            ...
            if not is_valid(temp_path):
                ...
                raise ValueError(f"the downloaded file is not valid ({size} bytes)")

            os.replace(temp_path, file_path)

            if is_valid(file_path):
                print(
                    f"   ✅ Font '{output_filename}' downloaded and verified."
                )
                return True

            raise FileNotFoundError(
                f"The final file '{output_filename}' is not valid in {font_dir}"
            )

        except Exception as e:
            print(
                f"   ⚠️ Font '{output_filename}' download attempt {attempt} failed: {e}"
            )
    ...
    print(f"   ❌ Gave up on font '{output_filename}' after {max_retry} attempts.")
    return False
...
def prepare_typography_fonts(cfg):
    ...
    primary_path = os.path.join(font_dir, f_primary["file"])
    accent_path = os.path.join(font_dir, f_accent["file"])

    if not (
        ok_primary and os.path.exists(primary_path) and os.path.getsize(primary_path) > 1000
    ):
        raise RuntimeError(f"Could not prepare the primary font: {primary_path}")
