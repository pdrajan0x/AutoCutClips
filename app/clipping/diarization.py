...
Detects distinct speakers in audio/video with the
pyannote/speaker-diarization-3.1 model. Used by the split-screen and
camera-switch podcast features.
"""
...
def run_diarization(
    ...
    if not hf_token:
        raise RuntimeError(
            ...
            "Set it via: export HF_TOKEN='your-token' or in a .env file"
        )

    print("🎙️ Loading the Pyannote speaker-diarization-3.1 model...")
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print("   ✅ Pyannote is using the GPU (CUDA)")
        else:
            print("   ℹ️ Pyannote is using the CPU")
    except Exception:
        print("   ℹ️ Pyannote is using the CPU")

    print("🎙️ Running speaker diarization...")
    if str(num_speakers).lower() == "auto":
        # Use min_speakers / max_speakers when they are provided.
        kwargs = {}
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to process the diarization result ({type(diarization)}): {e}. "
            "Make sure pyannote/speaker-diarization-3.1 is installed correctly."
        )
    ...
    print(
        f"   ✅ Diarization finished: {len(merged)} segments, {len(speakers)} speakers ({', '.join(speakers)})"
    )
